    ...
    public partial class ResourcesInfoDal : BaseDal<ResourcesInfo>, IResourcesInfoDal
    {
        public override DataTable GetListByPage(Hashtable ht, out int RowCount, bool IsPage, string where)
        {
            ...
            try
            {
                ...
                string OrderBy = ht["OrderBy"].SafeToString();
...
IDCard + "' where 1=1 ");
                ...
                {
                    sb.Append(" and '|'+re.ChapterID+'|' like '|" + ChapterID + "|%'");
                }
                ...
                dt = SQLHelp.GetListByPage("(" + sb.ToString() + ")", where, OrderBy, StartIndex, EndIndex, IsPage, null, out RowCount);
            }
            catch (Exception ex)
            {
