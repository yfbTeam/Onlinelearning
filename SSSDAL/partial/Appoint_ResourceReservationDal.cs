...
namespace SSSDAL
{
    public partial class Appoint_ResourceReservationDal
    {
        public override DataTable GetListByPage(Hashtable ht, out int RowCount, bool IsPage, string where)
        {
            ...
            try
            {
                ...
                sbSql4org.Append(@"select *  from Appoint_ResourceReservation rr where 1=1 and rr.IsDelete=0 and rr.UseStatus=0");
                if (ht.ContainsKey("ResourceTypeName") && !string.IsNullOrEmpty(ht["ResourceTypeName"].SafeToString()))
                {
                    sbSql4org.AppendFormat(" and rr.ReSourceInfoId in (select rri.Id from Appoint_ResourceReservationInfo rri where rri.ResourceTypeName='{0}' and rri.IsDelete=0  and rri.UseStatus=0)", ht["ResourceTypeName"].ToString());
                }
