            ...
            try
            {
                ...
                if (ht.ContainsKey("GetTime") && !string.IsNullOrEmpty(ht["GetTime"].SafeToString()))
                {
                    if (ht.ContainsKey("Name") && !string.IsNullOrEmpty(ht["Name"].SafeToString()))
                    {
                        ...
                        sbSql4org.AppendFormat(@"select * from Appoint_TimeManagement where TimeIntervalId in (
                        select rtm.TimeIntervalId from Appoint_ResourceTimeMappingId rtm 
                        inner join Appoint_ResourceReservationCla rrc on  
                ...
                else if (ht.ContainsKey("ValidateTime") && !string.IsNullOrEmpty(ht["ValidateTime"].SafeToString()))
                {
                    //解决bug319在会议室预约时间段冲突
                    StringBuilder sbSql4org = new StringBuilder();
                    sbSql4org.AppendFormat(@"select max(EndTime) as EndTime from Appoint_TimeManagement where TimeIntervalId in (
