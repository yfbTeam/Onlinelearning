...
namespace SSSWeb.SystemSettings
{
    ...
    public class VerificationCode : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            ////写入Session
            //context.Session["VerificationCode"] = chkCode;
            //写入Cookie
            context.Response.Cookies["VerificationCode"].Value = chkCode;
            //创建画布
            ...
            for (int i = 0; i < chkCode.Length; i++)
            {
                ...
                Color clr = color[rnd.Next(color.Length)];
                g.DrawString(chkCode[i].ToString(), ft, new SolidBrush(clr), (float)i * 18 + 2, (float)4);
            }
            //画噪点 
            for (int i = 0; i < 100; i++)
            {
