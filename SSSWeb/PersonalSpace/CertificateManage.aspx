<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CertificateManage.aspx.cs" Inherits="SSSWeb.PersonalSpace.CertificateManage" %>
...
<head>
    ...
    <script id="tr_Certificate" type="text/x-jquery-tmpl">
        <tr>
            ...
            <td>{{if Status==1}}
                {{else}}
                <a onclick="CheckCertificate(${ID})" style="cursor: pointer;">审核</a>
                {{/if}}
            </td>
        </tr>
    </script>
...
<script type="text/javascript">
    ...
    function EditCertificate(ID) {
        OpenIFrameWindow('修改平台证书', 'AddCertificate.aspx?ID=' + ID, '380px', '480px');
    }
    //证书
            ...
            success: function (json) {
                ...
                if (json.CertificateManage.errNum.toString() == "0") {
                    ...
                    $.each(json.CertificateManage.retData.PagedData, function (n, value) {
                        ...
                        var alink = "<a onclick=\"StuDetail(" + this.ID + ")\" style='cursor:pointer;'>学员信息</a>&nbsp;<a onclick=\"EditCertificate(" + this.ID +
                            ")\" style='cursor:pointer;'>证书修改</a>&nbsp;<a onclick=\"DelCertificate(" + this.ID + ")\" style='cursor:pointer;'>证书删除</a>&nbsp;<a onclick=\"ShowCertificate(" + this.ID + ",'" + encodeURI(CourseName)
                            + "','" + this.ImageUrl + "','" + encodeURI(this.Name) + "')\" style='cursor:pointer;'>证书预览</a>";
                        tr += "<tr> <td>" + this.Name + "</td><td>" + CourseName.trim('-') + "</td><td>" + DateTimeConvert(this.CreateTime, 'yyyy-MM-dd') + "</td><td>"
                        + alink + "</td>";
                    })
    ...
    function AddCertificate() {
        OpenIFrameWindow('新建平台证书', 'AddCertificate.aspx', '380px', '480px');
    }
    //学员信息
    function StuDetail(ID) {
        OpenIFrameWindow('学员信息', 'StuDetail.aspx?CertiID=' + ID, '480px', '460px');
    }
    function CheckCertificate(ID) {
        if (ID == "") {
            ...
            OpenIFrameWindow('证书申请审核', 'CheckApply.aspx?ID=' + ID, '540px', '330px');
        }
    }
    function ShowCertificate(ID, CourseName, ImageUrl, Name) {
        OpenIFrameWindow('证书预览', 'CertificateShow.aspx?ID=' + ID + "&CourseName=" + CourseName + "&ImageUrl=" + ImageUrl + "&Name=" + Name + "&Type=1", '480px', '480px');
    }
    function UploadAttach() {
        OpenIFrameWindow('上传附件', 'CertificateAttach.aspx?Type=2', '320px', '300px');
    }
</script>
</html>
