<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ResourceReservationInfo.aspx.cs" Inherits="SSSWeb.ResourceReservations.ResourceReservationInfo" %>
...
<body>
    ...
    <script>
        ...
        $(function () {
            ...
            //二级菜单选中
            $('#menu_side').find('li ul li').children('div').click(function () {
       ...
       /* function knotContentHover(obj) {
        ...
        function ClearActiveClass() {
            ...
            $("#menu_side  li ul li ul li").removeClass("active");
        }
        //获取数据
        var ids = "";
                ...
                $(json.result.retData).each(function () {
                    ...
                    if (pid == "0" && this.PId == pid) {
                        ...
                        chapterDiv += "<div class=\"item_chapter\" id='" + divid + "' data-name='" + this.Name + "' onclick=\"changeMenu(this," + this.Id + ")\" ><span>" + this.Name +
                            "</span><i class=\"icon  icon-angle-down\"></i>"
                            //+ caozuo
                            + "</div>";
                        i++;
                    ...
                    if (pid != "0" && this.PId == pid) {
                        caozuo = "<div class=\"btn-area\"><a href=\"javascript:void(0)\" onclick=\"EditMenu(this," + this.Id +
                         ",'" + this.Name + "')\">编辑</a>" + "<a href=\"javascript:void(0)\" onclick=\"DelMenu(" + this.Id + ")\">删除</a></div>";
                        chapterDiv += "<li><div class=\"" + Itemclass + "\" id='" + divid + "' data-name='" + this.Name + "' onclick=\"changeMenu(this,"
                            + this.Id + ")\"><span>" + this.Name + "</span>"
                        // + caozuo +
                        "</div>"
                        j++;
                        BindChapator(this.Id, this.PId, json);
                ...
                if (PName != "车辆") {
                    $.ajax({
                        ...
                        data: {
                            ...
                            "GetTime": "1", "Name": PName, "ispage": "false", "ResourceId": Pid, "ValidateTime": 1
                        },
                        success: function (json) {
                            if (json.result.errNum.toString() == "0") {
                                $(json.result.retData).each(function (i, n) {
                                    $("#HTime").attr("value", n.EndTime);
                                });
                    ...
                    if ($("#HTime").val() == "") {
                        layer.msg("该资源没有匹配时间段，请去时间段维护页面完善信息!");
                        return;
                    }
                }
                var maxTime = $("#HTime").val();

                OpenIFrameWindow('新增资源', 'ResourceAdd.aspx?PID=' + Pid + '&PName=' + escape(PName) + "&ParentID=" + UrlDate.ParentID + "&PageName=" + UrlDate.PageName + "&Time=" + maxTime, '700px', '80%');
