<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ExamPaperDetail.aspx.cs" Inherits="SSSWeb.ExamManage.ExamPaperDetail" %>
...
<script type="text/javascript">
    ...
    function getExamPaper() {
        $.ajax({
            ...
            async: false,
            dataType: "json",
            data: { PageName: "/ExamManage/PaperHandler.ashx", "action": "GetPaperById", ItemId: id },
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
                    ...
                    var html = "<div class=\"paper_title\">"
                                 + "<h2>" + model.Title + "</h2>"
                                 + "<p>满分" + model.FullScore + "分，考试时间" + model.ExamTime + "分钟。</p>"
                    "</div>";
                    $("#paperdiv").html(html);
                    MenuShow();
    ...
    function GetQNumByTypeObj(idd) {
        $.ajax({
            ...
            dataType: "json",
            data: {
                PageName: "/ExamManage/PaperHandler.ashx", "action": "GetQNumByTypePaperObj", "ExampaperID": idd
            },
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
    ...
    function GetQNumByTypeSub(idd) {
        $.ajax({
            ...
            dataType: "json",
            data: {
                PageName: "/ExamManage/PaperHandler.ashx", "action": "GetQNumByTypePaperSub", "ExampaperID": idd
            },
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
                    $.each(json.result.retData, function () {
    ...
    function GetQDetailOfObj(Title, idd) {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {
                PageName: "/ExamManage/PaperHandler.ashx", "action": "GetQDetailOfPaperObj", "ExampaperID": idd, "Title": Title
            },
            success: function (json) {
                $.each(json.result.retData, function () {
                    QCount++;
    ...
    function GetQDetailOfSub(Title, idd) {
        $.ajax({
            url: "../Common.ashx",//random" + Math.random(),//方法所在页面和方法名
            type: "post",
            async: false,
            dataType: "json",
            data: {
                PageName: "/ExamManage/PaperHandler.ashx", "action": "GetQDetailOfPaperSub", "ExampaperID": idd, "Title": Title
            },
            success: function (json) {
                $.each(json.result.retData, function () {
                    QCount++;
