<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AnsweredExam.aspx.cs" Inherits="SSSWeb.ExamManage.AnsweredExam" %>
...
<body style="background: #fff;">
    ...
    <form id="form1" runat="server">
        ...
        <input type="hidden" id="hSF" value="<%=UserInfo.UserType %>" />
        <!--header-->
...
<script type="text/javascript">
    ...
    var str = UrlDate.IsShow;// $('#hSF').val();
    var yue = "";
    $(function () {

        GetSameLiveMenu("ExamManage/MyExam.aspx", '<%=UserInfo.UniqueNo%>', '');
            ...
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
                    $.each(json.result.retData, function () {
                        status = this.Status;
                        GetQNumByTypeObj(this.exid, id);
                        GetQNumByTypeSub(this.exid, id);
                        var html = "<div class=\"paper_title\">"
                              + "<h2>" + this.name + "</h2>"
                              + "<p>试卷满分" + this.FullScore + "分，考试时间" + this.ExamTime + "分钟，";
                        if ($("#hSF").val() == "教师") {
                            html += "作答人：" + this.CreateUID + "，";
                        }
    ...
    //查询 客观题 题数及分数
    function GetQNumByTypeObj(exid, idd) {
        $.ajax({
            url: "../Common.ashx",//random" + Math.random(),//方法所在页面和方法名
            type: "post",
            async: false,
            ...
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
                    $.each(json.result.retData, function () {
                        ...
                        GetQTypeByTitle(this.Name, exid, 1);
                        fhr += "</div>";
                        fhr += "</div>";
                        fhr += "</div>";
                        $("#paperdiv1").html(fhr);
    ...
    //查询 主观题 题数及分数
    function GetQNumByTypeSub(exid, idd) {
        $.ajax({
            url: "../Common.ashx",//random" + Math.random(),//方法所在页面和方法名
            type: "post",
            async: false,
                    ...
                    $.each(json.result.retData, function () {
                        ...
                        htmll += "<div class=\"paper_mes\">"
                            + "<div class=\"paper_numer\">"
                                ...
                                + "</h1>"
                                + "<div class=\"paper_number_mes\">";
                        GetQTypeByTitle(this.Name, exid, 2);
                        htmll += "</div>";
                        htmll += "</div>";
                        htmll += "</div>";
    ...
    //根据题型名称获取题型id
    function GetQTypeByTitle(Title, exid, QType) {
        $.ajax({
            url: "../Common.ashx",//random" + Math.random(),//方法所在页面和方法名
            type: "post",
            async: false,
            ...
            data: {
                ...
                "action": "GetQuestionTypeList", "Title": Title
            },
            success: function (json) {
                if (json.result.errNum == "0") {
                    $(json.result.retData).each(function () {
                        if (QType == "1") //客观题
                        {
                            GetQDetailOfObj(this.ID, exid);
                        }
                        else {
                            GetQDetailOfSub(this.ID, exid);
                        }
                    });
                }
            }
    ...
    function GetQDetailOfObj(type, exid) {
        ...
        $.ajax({
            ...
            data: {
                PageName: "/ExamManage/ExamNationHander.ashx",
                "action": "GetAnswerListObj", "ExamID": exid, "Type": type
            },
            success: function (json) {
                $.each(json.result.retData, function () {
                    QCount++;
    ...
    function GetQDetailOfSub(type, exid) {
        $.ajax({
            url: "../Common.ashx",//random" + Math.random(),//方法所在页面和方法名
            type: "post",
            async: false,
            ...
            data: {
                PageName: "/ExamManage/ExamNationHander.ashx",
                "action": "GetAnswerListSub", "ExamID": exid, "Type": type
            },
            success: function (json) {
                examid = exid;
                typeid = type;
                ...
                $.each(json.result.retData, function () {
                    ...
                    if ('<%=UserInfo.UserType%>' != "2" && status == 1) {
