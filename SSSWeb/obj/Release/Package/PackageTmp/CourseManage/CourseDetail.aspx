...
<head>
    ...
    <script src="/CourseManage/TopicAndComment.js"></script>
    <style>
            ...
            <div class="discuss_description fl">
                ...
                <h1 class="clearfix">
                    ...
                    <div class="discuss-wrap fr">
                        <div class="fl comment" onclick="ShowComment(this)">
    ...
    <script id="tr_work" type="text/x-jquery-tmpl">
        <tr>
            ...
            <td>${ClassName}</td>
            <td>${DateTimeConvert(EidtTime,"yyyy-MM-dd HH:mm")}</td>
            <td><a onclick="DownLoad('${Attachment}');" style="cursor:pointer;">${CutFileName(Attachment)}</a></td>
            <td>{{if CorrectUID==''}}<span style="color: red;">否</span>{{else}}<span>是</span>{{/if}}</td>
            <td>${Score}</td>
            <td>${StoreLevel}</td>
            <td>
...
<body>
    ...
    <form id="form2" runat="server">
        ...
        <header class="repository_header_wrap manage_header">
            <div class="width repository_header clearfix">
                ...
                <nav class="navbar menu_mid fl">
                    ...
                    <li class="active"><a href="/CourseManage/MyCourceManage.aspx">我的课程</a></li>
                    </ul>
                </nav>
        ...
        <div class="onlinetest_item width pt90">
            <div class="myexam bordshadrad">
                ...
                <!--面包屑-->
                <div class="crumbs">
                    <a id="Mycource"  style="cursor:pointer;" onclick="javascript:window.history.go(-1)">我的课程</a>
                    <span>></span>
                    <a href="#" id="CourceName"></a>
                     <%--<object classid="CLSID:8856F961-340A-11D0-A96B-00C04FD705A2" height="0" width="0" id="WebBrowser"></object>--%>
                    <%--<asp:Button ID="Button1" runat="server" Text="静态课程" OnClick="Button1_Click" />--%>
                </div>
                <div class="mycourse">
        ...
        <div class="width coursedetail_wrap mb10 bordshadrad">
                    ...
                    <div class="course_detail_listswrap">
                        <div>
                            <div class="stytem_select_right fr">
                                <a onclick="SelResource(1)" style="cursor: pointer;  padding:8px 18px;">选择视频</a>
                                <a href="javascript:;" onclick="AddWeike(1)" style="cursor: pointer; padding:8px 18px;">上传视频</a>
                            </div>
                            <div class="clear">
                            </div>
                        ...
                        <div class="none">
                            <div class="stytem_select_right fr">
                                <a onclick="SelResource(0)" style="cursor: pointer; padding:8px 18px;">选择资源</a>
                                <div onclick="$('#uploadify').click();" style="border-radius: 2px; background: #1472b9; top: 67px; width: 90px; height: 30px; text-align: center; right: 120px; color: rgb(255, 255, 255); font-size: 12px; display: block; position: absolute; z-index: 2; cursor: pointer;" class="un_reposity">
                        ...
                        <div class="none">
                            <div class="stytem_select_right fr">
                                <a href="javascript:void(0);" id="a_AddWork" onclick="AddWork();" style="cursor: pointer;">发布作业</a>
                            </div>
                            <div class="clear"></div>
            ...
            <div class="coursedetail clearfix pr coursedetaila" id="zuoye">
                ...
                <div>
                    ...
                    <h1 class="course_detail clearfix">
                        <div class="fl" style="padding:10px 0px;">
                            <i class="icon">
                                <img src="/images/homework.png" alt="" /></i>
                            <span style="color: #1775bd">作业</span>                            
                        </div>
                            <a href="javascript:void(0);" onclick="LookCourseWork_Click();" class="btn" style="cursor:pointer;width:150px;float:right;height:25px;border:none;line-height:25px;background:#1775BD;">查看学生提交作业信息</a>
                    </h1>
    ...
    <script type="text/javascript">
        function ShowComment(em) {
            $(em).parents('li').find('.comment_wrap').toggle();
        }
        function ExportCouse() {
            //页面层
            layer.open({
                type: 1,
                skin: 'layui-layer-rim', //加上边框
                area: ['420px', '240px'], //宽高
                content: '<div><input type=\"radio\" name=\"chkCustomModel\" value=\"0\" />模板1<input type=\"radio\" name=\"chkCustomModel\" value=\"1\" />模板2<input type=\"button\"  value=\"确定\" id=\"CustomExport\" /></div>'
            });

            $("#CustomExport").on("click", function () {
                //loading层
                ...
                $.ajax({
                    url: "/SysMessage/StaticPage.ashx",
                    type: "post",
                    data: { url: url, cid: $("#HUserIdCard").val(), uname: $("#HUserName").val() },
                    dataType: "json",
                    success: function (json) {
                        if (json.result == "ok") {
                            layer.msg("生成成功！");
                            $("#PostCourseStatic").parent().attr("download", json.path);
                            $("#PostCourseStatic").parent().attr("href", json.path);
                            $("#PostCourseStatic").trigger("click");
                        } else {
                            layer.msg("生成失败！");
                        }
            ...
            if (json.result.errNum.toString() == "0") {
                ...
                $(json.result.retData).each(function () {
                    ...
                    var caozuo = "<div class=\"btn-area\"><a href=\"javascript:void(0)\" onclick='addLeftMenu(" +
                        ...
                        ")\">删除</a><a href=\"javascript:void(0)\" onclick=\"SortMenu(" + this.ID + ",'up')\">上移</a><a href=\"javascript:void(0)\" onclick=\"SortMenu(" + this.ID +
                        ",'down')\">下移</a></div>";
                    //if (Itemclass == "item_content") {
                ...
                success: function (json) {
                    if (json.result.errNum.toString() == "0") {
                        ...
                        "任务：" + this.RelName + "( 权重：" + this.Weight + "&emsp; 学生范围：" + this.ClassStrName + ")</div>" + " <div class='clearfix mt5'><div class=\"clearfix fl caozuo_none\">"
                            + ("<div class=\"fl\" style=\"color: #0b70bc\" onclick=\"DeleteTask('" + this.ID + "','false');\"><i class=\"icon icon-trash\" style=\"color: #0b70bc; display: inline-block;\"></i>删除 </div>") + "</div></div></div>" +
        ...
        function AddWeike(IsVideo) {
            ...
            OpenIFrameWindow('新增微课', 'AddWeike.aspx?CourceID=' + CourceID + "&ChapterID=" + $("#ChapterID").val() + "&IsVideo=" + IsVideo + "&UserIdCard=" + UserIdCard, '600px', '430px');
        }
        ...
        function LookCourseWork_Click() {
            OpenIFrameWindow('查看提交作业信息', '/OnlineLearning/LookStuWorkStatistics.aspx?courseid=' + UrlDate.itemid + "&coursetype=" + $(".course_type").attr("typevalue"), '580px', '450px')
        }
