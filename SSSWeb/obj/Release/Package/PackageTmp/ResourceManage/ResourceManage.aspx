<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ResourceManage.aspx.cs" Inherits="SSSWeb.ResourceManage.ResourceManage" %>
...
<script type="text/javascript">
    ...
    function Down(id) {
        ...
        if (ids != "") {
            ...
            $.ajax({
                url: "/ResourceManage/ResourceHander.ashx",//random" + Math.random(),//方法所在页面和方法名
                type: "post",
                async: false,
                dataType: "json",
                data: { "PageName": "ResourceManage/ResourceHander.ashx", "Func": "Down", DownID: ids },
                success: function (json) {
                    if (json.result.errNum.toString() == "0") {
    ...
    function Move(id) {
        ...
        if (ids == "") {
            ...
            OpenIFrameWindow('文件移动', '/ResourceManage/ResourceContent.aspx?Pid=' + pid + "&code=" + Pcode + "&id=" + id + "&random=" + Math.random(), '450px', '300px;');
        }
    }
    function MoveMore(url, pid, code, id) {
        ...
        $.ajax({
            ...
            data: {
                "PageName": "ResourceManage/ResourceHander.ashx",
        ...
        if (ids != "") {
            ...
            if (confirm("确定要删除'" + name + "'吗？")) {
                $.ajax({
                    ...
                    async: false,
                    dataType: "json",
                    data: { "PageName": "ResourceManage/ResourceHander.ashx", "Func": "Del", DelID: ids },
                    success: function (json) {
                        if (json.result.errNum.toString() == "0") {
                            layer.msg("删除成功");
                            getData(1, 10);
    ...
    function upload() {
        ...
        OpenIFrameWindow('文件上传', '/ResourceManage/ResouceUpload.aspx?FoldUrl=' + FoldUrl + "&Pid=" + pid + "&code=" + Pcode + "&CreateUID=" + CreateUID, '400px', '300px');
    }
    //格式化时间
