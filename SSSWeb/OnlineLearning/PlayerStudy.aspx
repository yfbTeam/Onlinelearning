<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlayerStudy.aspx.cs" Inherits="SSSWeb.OnlineLearning.PlayerStudy" %>
...
<head>
    ...
    <script id="li_mylessons" type="text/x-jquery-tmpl">
        <dt class="fl">
            ...
            <p class="onlinestudy_teacher">讲师：<span>${BatchUserName}</span></p>
...
<body>
    ...
    <input type="hidden" id="HStuIDCard" value="<%=UserInfo.UniqueNo%>" />
    <input type="hidden" id="Hid_ClassID" value="<%=UserInfo.UserType %>" />
    <!--header-->
    ...
    <div class="onlinestudy_wrap clearfix">
        <div class="onlinestudy_left fl">
            ...
            <div id="div_OutVideo" class="videobox">
                <div id="playVideo" style="width:100%;height:100%;"></div>
            ...
            <div class="menu_wrap pr">
                <div class="menu_sidewrap">
                    ...
                    <div class="data_discuss none">
                        ...
                        <a href="javascript:;" id="a_topicLookAll" class="seeall" onclick="LookAllData(1,'a_topicLookAll','all');">查看全部</a>
                    </div>
                    <div class="data_note none">
                        ...
                        <a href="javascript:;" id="a_noteLookAll" class="seeall" onclick="LookAllData(2,'a_noteLookAll','all');">查看全部</a>
                    </div>
                    <div class="data_work  none">
                        <div class="forumsel_wrap">
                            ...
                            <a href="javascript:;" id="a_taskLookAll" class="seeall" onclick="LookAllData(3,'a_taskLookAll','all');">查看全部</a>
                        </div>
                    </div>
                </div>
                <div class="menu_nav">
                    <ul>
                        ...
                        <li><a href="javascript:;" onclick="BindPutongResource();">文件</a></li>
                        <li><a href="javascript:;" onclick="LookAllData(1,'a_topicLookAll','top5');">讨论</a></li>
                        <li><a href="javascript:;" onclick="LookAllData(2,'a_noteLookAll','top5');">笔记</a></li>
                        <%--<li><a href="javascript:;" onclick="LookAllData(3,'a_taskLookAll','top5');">任务</a></li>--%>
                    </ul>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function BindWeikeResource(playid, type) {
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
                    $(json.result.retData).each(function () {
                        videoArrayList.push(this.AbsFileUrl);
                        var video_li = " <li title=\"" + this.Name + "\" id=\"playV_" + this.ID + "\" playindex=\"" + (videoArrayList.length - 1) + "\" playname=\"" + this.Name + "\" fileurl=\"" + this.AbsFileUrl + "\" clickid=" + this.ClickId + " clicknum=" + this.ClickNum + " watchtime=" + this.WatchTime + " islookend=" + this.IsLookEnd + " onclick=\"SetPlayValue(this.id);\">" + videoArrayList.length + "</li>";
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
                    ...
                    SetPlayValue("playV_" + playid, type);
    ...
    function SetPlayValue(objid) {
        ...
        if (window.SewisePlayer) {
            if (SewisePlayer.doStop != undefined) {
                SewisePlayer.doStop();
            }            
        }
        $("#div_OutVideo").empty();
        $("#div_OutVideo").html('<div id="playVideo"  style="width:100%;height:100%;"></div>');        
        var extindex = url.lastIndexOf('.');
        var curtype = url.substring(extindex+1);
        var befurl = url.substring(0, extindex + 1);
        var curskin = '';
        if (curtype != "mp4" && curtype != "ogg" && curtype != "webm") {
            curskin = 'vodOrange';
        }
        SewisePlayer.setup({
            server: "vod",
            ...
            skin: curskin,
            logo: "/Scripts/sewise-player/html/skins/vodWhite/logo.png",
        ...
        //SewisePlayer.toPlay(url, name, watchtime, true);
        SewisePlayer.onMetadata(function (meta, id) {
            curTotalTime = SewisePlayer.duration();  //获取总时长                     
        });
        if (hasEvent == 0) {
            hasEvent = 1;
            SewisePlayer.onPlayTime(function (time, id) {
                currentTime = time;//获取当前播放时间 
                if (SewisePlayer.player == "flash" && curTotalTime != undefined && curTotalTime != 0 && currentTime >= curTotalTime - 1) {
                    curIsend = 1;
                    currentTime = 0;
                    PlayVideoClick("next");
                }
        ...
        if (hasEnded == 0 && SewisePlayer.video) {
            hasEnded = 1;
            SewisePlayer.video.addEventListener('ended', function () {                
                curIsend = 1;
                currentTime = 0;
                PlayVideoClick("next");
            }, false);
    ...
    function moveCheck() {
        ...
        if (curPlayingVideo > 4) {
            var l = -(curPlayingVideo-4) * $width + 'px';
    ...
    function BindPutongResource() {
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.result.errNum.toString() == "0") {
                    $(json.result.retData).each(function () {
                        var li = " <li class=\"clearfix\"><i class=\"ico-" + this.postfix.toString().substring(1) + "-min h-ico\"></i><img style=\"width: 20px; height: 16px; float: left; margin-right: 2px;\"><p class=\"repository_name fl\">" +
                this.Name + this.postfix + "</p><div class=\"repository_download none\" onclick=\"DownLoad('" + this.AbsFileUrl + "');\"> <i class=\"icon icon-download-alt\"></i></div><span class=\"repository_date fr\">" +
...
<script type="text/javascript">
    ...
    function LookAllData(index, objid, type) {
        if (type == "all" || (type == "top5" && $('#' + objid).css('display') == 'none')) {
            if (index == 1) {
                GetTopicDataNotPage();
            } else if (index == 2) {
                GetTopicDataNotPage(1, "ul_note", "li_note", "comment_note", "li_notecomment", "span_notereplaycount");
            } else {
                BindCourseTaskNotPage();
            }
            $('#' + objid).hide();
        } else {
            if (index == 1) {
