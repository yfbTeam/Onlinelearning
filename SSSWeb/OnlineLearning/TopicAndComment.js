        ...
        success: function (json) {
            if (json.result.errNum.toString() == "0") {
                if (topicid != "") {
                    ...
                    //评论显示
                    $('.comment' + comtype).click(function () {
...
function DynamicCreateKindEditor(ulid) {
    ...
    $AllArea.each(function (index, item) {
        ...
        kinditerArry[kinditerindex] = K.create('#' + objId, {
            ...
            items: [
	            'fontname', '|', 'forecolor', 'hilitecolor', 'bold', 'italic', 'underline', "strikethrough",
	            'removeformat', '|', 'justifyleft', 'justifycenter', 'justifyright', 'insertorderedlist',
	            'insertunorderedlist', '|', 'undo', 'redo', '|', 'emoticons'],
            resizeType: 0,
...
function GetWorkData(ulid, liid, uploadifycss, prefix_id) {
    ulid = arguments[0] || "ul_tabwork";
    liid = arguments[1] || "li_tabwork";
    uploadifycss = arguments[2] || ".uploadify_work";
    prefix_id = arguments[3] || "uploadify_";
    $("#" + ulid).children().remove();
    $.ajax({
        url: "../Common.ashx",//random" + Math.random(),//方法所在页面和方法名
        type: "post",
        ...
        success: function (json) {
            ...
            if (json.result.errNum.toString() == "999") {
                ...
                $("#li_tabwork").tmpl(json.result.retData).appendTo("#ul_tabwork");
            ...
            else { $("#CountTask").html(0); $("#ul_tabwork").html("<li style=\"border:none;\">暂无作业！</li>"); }
        },
        error: function (errMsg) {
            ...
            if (upworkid != "") {
                ...
                $("#ul_catalogwork").children().each(function () {
                    ...
                    if (wwli.children().length == 0) {
                        wwli.append("<tr><td colspan='100' text-align='center'>" + (user_pagetype == "stu" ? "您还未提交作业" : "暂无提交作业的员工") + "</td></tr>");
                    }
                });
                //作业折叠
...
var studyTheCourseStaff = [], studyTheCourseStaff_Count = 0, comwork_stu = [];
//绑定作业下提交的数据
        ...
        success: function (json) {
            if (upworkid != "") {
                ...
                $("#ul_tabwork").children().each(function () {
                    ...
                    $("#lbl_nocommit_" + liid).html(studyTheCourseStaff_Count - parseInt($("#lbl_commit_" + liid).html()));
                    if (wwli.children().length == 0) {
                        wwli.append("<tr><td colspan='100' text-align='center'>" + (user_pagetype == "stu" ? "您还未提交作业" : "暂无提交作业的员工") + "</td></tr>");
                    }
                });
    ...
    $.ajax({
        ...
        success: function (json) {
            if (json.result.errNum.toString() == "0") {
                ...
                $("#lbl_nocommit_" + upworkid).html(studyTheCourseStaff_Count - workRelCount);
            }
            else {
                $("#lbl_commit_" + upworkid).html(0);
                $("#lbl_nocommit_" + upworkid).html(studyTheCourseStaff_Count);
            }
        },
        error: function (errMsg) {
            $("#lbl_commit_" + upworkid).html(0);
...
function UploadFile(uploadifycss, prefix_id, li_work) {
    $(uploadifycss + "[name='uploadify']").each(function () {
        ...
        WebUploader.create({
            pick: '#' + this.id,
            formData: { Func: "UplodeCourse_Work" },
            accept: {
                title: 'Images',
...
//查看作业
function LookCurWork(workid) {
    OpenIFrameWindow('查看作业', 'Course_WorkLook.aspx?itemid=' + workid + '&courseid=' + UrlDate.itemid, '500px', '410px');
}
//下载作业
...
function CorrectWorkClick(workid, workrelid) {
    upworkid = workid;
    OpenIFrameWindow('作业批改', 'WorkCorrectEdit.aspx?itemid=' + workrelid + "&flag=0", '580px', '360px');
}
function LookCorrectWork(workid, workrelid) {
    OpenIFrameWindow('详情批语', 'WorkCorrectEdit.aspx?itemid=' + workrelid + "&flag=1", '580px', '360px');
}
//删除作业
    ...
    $.ajax({
        ...
        success: function (json) {
            if (json.result.errNum.toString() == "0") {
                ...
                $('#ul_stuContent').html("<li><span>无未完成作业的员工!</span></li>");
            }
            layer.open({
                type: 1,
                shade: false,
