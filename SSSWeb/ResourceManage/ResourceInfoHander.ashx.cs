...
namespace SSSWeb.ResourceManage
{
    ...
    public class ResourceInfoHander : IHttpHandler
    {
        ...
        private void reName(HttpContext context)
        {
            ...
            Resources_Open modol = new Resources_Open();
        ...
        private void AddFolder(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    SSSModel.Resources_Open re = new SSSModel.Resources_Open();
                    re.Name = FileName;
                    re.CheckUsers = context.Request["CheckUsers"].SafeToString();
                    re.FileSize = 0;
                    re.FileUrl = RealPath;
        ...
        private void Down(HttpContext context)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < idarry.Length; i++)
                {
                    ...
                    SSSModel.Resources_Open resource = (SSSModel.Resources_Open)(model.retData);
        ...
        private void Del(HttpContext context)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < idarry.Length; i++)
                {
                    ...
                    #endregion
                    //数据删除
                    int num = Bll.Del(idarry[i].SafeToString());
                    if (num > 0)
                    {
                        jsonModel = new JsonModel()
                        {
            ...
            try
            {
                ...
                foreach (string id in idarry)
                {
                    if (id != "")
                    {
                        ...
                        Resources_Open resource = (Resources_Open)(model.retData);
                        string FileUrl = resource.FileUrl;
