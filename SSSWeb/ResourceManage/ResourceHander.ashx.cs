...
{
    /// <summary>
    /// ResourceHander 的摘要说明
    /// </summary>
    public class ResourceHander : IHttpHandler
    {
        ...
        private void EditVideo(HttpContext context)
        {
            JavaScriptSerializer jss = new System.Web.Script.Serialization.JavaScriptSerializer();

            string result = "";
            string Msg = "";
            //                        func: "EditVideo", ID: ID, NewName: NewName, FileUrl: UrlDate.Url, oldName: UrlDate.Name, VideoImg: VideoImg
            Couse_ResourceService bll = new Couse_ResourceService();
            int ID = Convert.ToInt32(context.Request["ID"]);
            string VideoImg = context.Request["VideoImg"];
            JsonModel jsonModel = bll.GetEntityById(ID);
            Couse_Resource entity = (Couse_Resource)jsonModel.retData;
            entity.VidoeImag = VideoImg;
            entity.ID = ID;
            int ResourcesID = Convert.ToInt32(entity.ResourcesID);
            ...
            if (Name != oldName)
            {
                ...
                LogService.WriteLog("FileUrl:" + FileUrl + "newUrl:" + newUrl);
                modol.ID = ResourcesID;
        ...
        private void AddFolder(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    SSSModel.Resource re = new SSSModel.Resource();
                    re.Name = FileName;
        ...
        private void Down(HttpContext context)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < idarry.Length; i++)
                {
                    ...
                    SSSModel.Resource resource = (SSSModel.Resource)(model.retData);
            ...
            try
            {
                ...
                foreach (string id in idarry)
                {
                    if (id != "")
                    {
                        ...
                        Resource resource = (Resource)(model.retData);
                        string FileUrl = resource.FileUrl;
