    ...
    public class Uploade : IHttpHandler
    {
        ...
        private void UplodLogo(HttpContext context)
        {
            ...
            try
            {
                ...
                DirectoryInfo di = new DirectoryInfo(Path.GetDirectoryName(context.Server.MapPath("/images/")));
                ...
                */
                ...
                #endregion
                FileHelper.DeleteFile(context.Server.MapPath("/images/" + Name));
                file.SaveAs(path);
                if (Name != fileName)
                {
        ...
        private void UplodExcel(HttpContext context)
        {
            ...
            ////{
            ////    int rowAffected = ExcelRender.RenderToDb(new MemoryStream(fileBytes), "insert into test(aa,bb,cc)", SQLHelp.ExecuteNonQuery);
        ...
        public void SetNoticeImg(HttpContext context)
        {
            string result = "";
            HttpPostedFile file = HttpContext.Current.Request.Files[0];
            string Fpath = ConfigHelper.GetConfigString("FileManageName") + "/Attatchment/Notice"; //"/images/MenuImage";
            FileHelper.CreateDirectory(context.Server.MapPath(Fpath));
            string ext = System.IO.Path.GetExtension(file.FileName);
            string fileName = DateTime.Now.Ticks + ext;
            string p = Fpath + "/" + fileName;
