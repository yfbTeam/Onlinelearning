...
namespace SSSWeb.Hander
{
    ...
    public class ModelHander : IHttpHandler
    {
                ...
                try
                {
                    switch (FuncName)
                    {
                        ...
                        case "GetSubButtonByUrl":
                            ...
                            break;
                        case"GetSameLiveMenuByPid":
                            GetSameLiveMenu(context);
                            break;
                        default:
        ...
        private void GetSameLiveMenu(HttpContext context)
        {
            try
            {
                jsonModel = bll.GetSameLiveMenuByPid(context.Request["Pid"], context.Request["LinkUrl"], context.Request["UniqueNo"]);
            }
            catch (Exception ex)
            {
                jsonModel = new JsonModel()
