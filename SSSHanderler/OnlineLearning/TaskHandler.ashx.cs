    ...
    public class TaskHandler : IHttpHandler
    {
        ...
        private void GetTaskDataPage(HttpContext context)
        {
            try
            {
                ...
                jsonModel = common.AddCreateNameForData(taskService.GetPage(ht, ispage), ispage,"StuRange");                
            }
            catch (Exception ex)
            {
                jsonModel = new JsonModel()
        ...
        private void GetCourseStuTaskInfo(HttpContext context)
        {
            try
            {
                ...
                if (jsonModel.errNum == 0)
                {
                    ...
                    List<Dictionary<string, object>> list =!string.IsNullOrEmpty(task.StuRange)?common.GetUnifiedUserData(task.StuRange):new List<Dictionary<string, object>>();
        ...
        private void GetComCountByTaskID(HttpContext context)
        {
            try
            {
                ...
                int taskid = Convert.ToInt32(context.Request["TaskID"] ?? "");
                allCount = new Couse_SelstuinfoService().GetLearnStaffCountByCourceID(context.Request["CourceID"]);
        ...
        private void GetStuTaskCompleteInfo(HttpContext context)
        {
            try
            {
                ...
                string staffNos = new Couse_SelstuinfoService().GetLearnStaffUniqueNosByCourceID(context.Request["CourceID"]);
                if (!string.IsNullOrEmpty(staffNos))
                {
                    list = new CommonHandler().GetUnifiedUserData(staffNos);
                }                
                Hashtable ht = new Hashtable();
                ht.Add("TaskID", taskid);
